...
class HubConfig 
{
     ...
     * @default null
     * @param ParamConfigurator|mixed $value
     * @deprecated Since symfony/mercure-bundle 0.3: The child node "jwt_provider" at path "mercure.hubs." is deprecated, use "jwt.provider" instead.
     * @return $this
     */
    ...
    public function __construct(array $config = [])
    {
        ...
        if (array_key_exists('jwt', $config)) {
            $this->_usedProperties['jwt'] = true;
            $this->jwt = \is_array($config['jwt']) ? new \Symfony\Config\Mercure\HubConfig\JwtConfig($config['jwt']) : $config['jwt'];
            unset($config['jwt']);
        }

        if (array_key_exists('jwt_provider', $config)) {
            $this->_usedProperties['jwtProvider'] = true;
            $this->jwtProvider = $config['jwt_provider'];
            unset($config['jwt_provider']);
        }
